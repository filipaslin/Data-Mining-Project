...
class BPRMF(object):
    def __init__(self, data_config):
        ...
        self.pscore_pos = tf.layers.dense(item_p,1, activation=tf.nn.sigmoid)
        self.clip = 0.01
    ...
    def _init_weights(self):
        ...
        items = np.loadtxt(
            'Data/' + args.dataset + '/items_lda_attribute.txt',
            delimiter=' ')
        users = np.loadtxt(
            'Data/' + args.dataset + '/users_attribute.txt',
            delimiter=' ')
        users = users[:, 1:]
...
if __name__ == '__main__':
    ...
    np.save('Data/' + args.dataset + '/user_embedding_neural.npy', arr=user_embedding / 5)
