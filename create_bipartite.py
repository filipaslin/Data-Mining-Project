...
directory = '/home/filipaslin/Data_Mining/Project/Causal-Understanding-of-Fake-News-Dissemination/'

f = open(directory + "Data/all_users_attribute.pkl","rb")
users=pickle.load(f)
#users = {key: users[key] for key in list(users.keys())[:100]}

f.close()
edges_list=[]
news_list=[]
news=pd.read_csv(directory + 'Data/dEFEND data(5)/dEFEND data/gossipcop_content_no_ignore.tsv', sep='\t', header=0)
...
for item in users.keys():
    ...
    for new_id in user_news:
        ...
        #News is fake
            ...
            if item not in user_fake.keys():
                user_fake[item] = 1
            else:
                user_fake[item] += 1
        # News is true
        elif news[new_id][0]==0:
            if item not in user_true.keys():
                user_true[item] = 1
            else:
                user_true[item] += 1
...
graphs = list(B.subgraph(c) for c in nx.connected_components(B))[0]
print(len(graphs))
...
id_new=0
id_u=0
f1=open(directory + 'Data/Gossip/users_list.txt','w')
f2=open(directory + 'Data/Gossip/news_list.txt','w')
f1.write('org_id remap_id\n')
f2.write('org_id remap_id\n')
f3=open(directory + 'Data/Gossip/user_label.txt','w')
...
f1=open(directory + 'Data/Gossip/test.txt','w')
f_pscore=open(directory + 'Data/Gossip/test-pscore.txt','w')
...
f2=open(directory + 'Data/Gossip/train.txt','w')
f_pscore=open(directory + 'Data/Gossip/train-pscore.txt','w')
...
np.savetxt(directory + 'Data/Gossip/items_lda_attribute.txt',X_topic)

f3=open(directory + 'Data/Gossip/users_attribute.txt','w')
...
for u in users.keys():
    if u in user_nodes:
        ...
        if u in user_fake:
            attributes.append(user_fake[u])
        else:
            attributes.append(0)
        if u in user_true:
            attributes.append(user_true[u])
        else:
            attributes.append(0)
